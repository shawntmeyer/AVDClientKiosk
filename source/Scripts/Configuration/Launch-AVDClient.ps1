...
If ($AutoLogon -or $Yubikey) {
    If ($AutoLogon -and $Yubikey) {
        ...
        $Query = "SELECT * FROM __InstanceDeletionEvent WITHIN 2 WHERE TargetInstance ISA 'Win32_PnPEntity' AND (TargetInstance.PNPDeviceID LIKE 'USB%VID_1050%' OR TargetInstance.PNPDeviceID LIKE 'SCFILTER%')"
        $SourceIdentifier = "Remove_YUBIKEY_or_SMARTCARD_Event"
    } Elseif ($AutoLogon -and !$Yubikey) {
        $Query = "SELECT * FROM __InstanceDeletionEvent WITHIN 2 WHERE TargetInstance ISA 'Win32_PnPEntity' AND TargetInstance.PNPDeviceID LIKE 'SCFILTER%'"
        $SourceIdentifier = "Remove_SMARTCARD_Event"
    } Elseif (!$AutoLogon -and $Yubikey) {
        $Query = "SELECT * FROM __InstanceDeletionEvent WITHIN 2 WHERE TargetInstance ISA 'Win32_PnPEntity' AND TargetInstance.PNPDeviceID LIKE 'USB%VID_1050%'"
