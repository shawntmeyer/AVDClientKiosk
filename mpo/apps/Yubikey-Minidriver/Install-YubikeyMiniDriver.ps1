...
$Url = 'https://downloads.yubico.com/support/YubiKey-Minidriver-latest-x64.msi'
...
If ($DeploymentType -ne 'UnInstall') {
    ...
    If (-not $MSIPath) {
        ...
        $MSIPath = Get-InternetFile -Url $Url -OutputDirectory $TempDir -OutputFileName 'Yubikey-Minidriver.msi'
    }
    If ($MSIPath) {    
        Write-Output "Installing '$SoftwareName' via cmdline:"
        Write-Output "     'msiexec.exe /i `"$MSIPath`" /qn'"
        $Installer = Start-Process -FilePath 'msiexec.exe' -ArgumentList "/i `"$MSIPath`" /qn" -Wait -PassThru
    ...
    Else {
        Write-Error "$SoftwareName installer not found"
        Exit 1
    }
}
# Uninstall
