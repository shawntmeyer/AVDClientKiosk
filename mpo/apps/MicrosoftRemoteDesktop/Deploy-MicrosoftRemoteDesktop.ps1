...
$CustomShortcutPath = "$env:AllUsersProfile\Microsoft\Windows\Start Menu\Programs\AVD.lnk"
...
If ($DeploymentType -ne 'UnInstall') {
    ...
    Else {
        Throw "The Installer exit code is $($Installer.ExitCode)"
    }
    Write-Output "Completed '$SoftwareName' Installation."
    # Wait up to 20 seconds for the default shortcut to exist, then copy it
    $defaultShortcut = "$env:AllUsersProfile\Microsoft\Windows\Start Menu\Programs\Remote Desktop.lnk"
    $waitTime = 0
    while (-not (Test-Path $defaultShortcut) -and $waitTime -lt 20) {
        Start-Sleep -Seconds 1
        $waitTime++
    }
    if (Test-Path $defaultShortcut) {
        Copy-Item -Path $defaultShortcut -Destination $CustomShortcutPath -Force
    } else {
        Throw "Default shortcut not found after 20 seconds."
    }
...
Else {
    ...
    If (Test-Path -Path $CustomShortcutPath) {
        Remove-Item -Path $CustomShortcutPath -Force -ErrorAction SilentlyContinue
    }
}
Stop-Transcript